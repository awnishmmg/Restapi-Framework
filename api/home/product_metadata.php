...
function product_metadata_get($key='')
{
	...
	else if($key)
	{
		...
		//$chain = true;
		$data = poly_join("tbl_product_meta_data=prod_meta_id,product_id as product_meta_product_id,in_offer_id as product_meta_offer_id,is_metakey_id as product_meta_metakey_id,rel_pair as product_meta_rel_pair|tbl_product=product_id as pid,product_name,product_old_price,product_current_price,product_qty,product_net_wt,product_gros_wt,product_featured_photo,product_description,product_short_description,product_feature,product_condition,product_return_policy,product_total_view|tbl_product_to_meta_key=metakey_name,description as meta_description,status as meta_satus,sort_order",['product_id','is_metakey_id'],'left',['tbl_product_to_meta_key.metakey_name'=>['=',$key]]);
		//prx($data);
		json_bind($data,CONST_HTTP_STATUS_OK,'All Record ',true);
	}
